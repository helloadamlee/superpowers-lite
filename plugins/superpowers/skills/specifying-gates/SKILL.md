...
Batch independent questions into the fewest supported round-trips. Ask a later
question separately only when its options genuinely depend on an earlier answer
(for example, ask the subagent dispatch contract only after that mechanism is
selected). Do not impose one-question-at-a-time ceremony.
...
AskUserQuestion:
  ...
  options:
    ...
    - label: "Subagent with briefing"
      description: "Dispatch the exact Codex role selected by modelTier with a prompt template"
